...
class ResBlock(tf.keras.layers.Layer):
    def __init__(self, channels, emb_channels, out_channels):
        ...
        self.in_layers = [tfa.layers.GroupNormalization(epsilon = 1e-5), tf.keras.activations.swish, get_conv2d(channels, out_channels, 3, padding = 1)]
        self.emb_layers = [tf.keras.activations.swish, tf.keras.layers.Dense(out_channels)]
        self.out_layers = [tfa.layers.GroupNormalization(epsilon = 1e-5), tf.keras.activations.swish, lambda x: x, get_conv2d(out_channels, out_channels, 3, padding = 1)]
...
class FeedForward(tf.keras.layers.Layer):
    def __init__(self, dim, mult = 4):
        ...
        self.net = [GEGLU(dim, dim * mult), lambda x: x, tf.keras.layers.Dense(dim)]
...
class UNetModel(tf.keras.models.Model):
    def __init__(self):
        ...
        self.time_embed = [tf.keras.layers.Dense(1280), tf.keras.activations.swish, tf.keras.layers.Dense(1280), ]
        self.input_blocks = [[get_conv2d(4, 320, kernel_size = 3, padding = 1)], [ResBlock(320, 1280, 320), SpatialTransformer(320, 768, 8, 40)], [ResBlock(320, 1280, 320), SpatialTransformer(320, 768, 8, 40)], [Downsample(320)], [ResBlock(320, 1280, 640), SpatialTransformer(640, 768, 8, 80)], [ResBlock(640, 1280, 640), SpatialTransformer(640, 768, 8, 80)], [Downsample(640)], [ResBlock(640, 1280, 1280), SpatialTransformer(1280, 768, 8, 160)], [ResBlock(1280, 1280, 1280), SpatialTransformer(1280, 768, 8, 160)], [Downsample(1280)], [ResBlock(1280, 1280, 1280)], [ResBlock(1280, 1280, 1280)]]
        self.middle_block = [ResBlock(1280, 1280, 1280), SpatialTransformer(1280, 768, 8, 160), ResBlock(1280, 1280, 1280)]
        self.output_blocks = [[ResBlock(2560, 1280, 1280)], [ResBlock(2560, 1280, 1280)], [ResBlock(2560, 1280, 1280), Upsample(1280)], [ResBlock(2560, 1280, 1280), SpatialTransformer(1280, 768, 8, 160)], [ResBlock(2560, 1280, 1280), SpatialTransformer(1280, 768, 8, 160)], [ResBlock(1920, 1280, 1280), SpatialTransformer(1280, 768, 8, 160), Upsample(1280)], [ResBlock(1920, 1280, 640), SpatialTransformer(640, 768, 8, 80)],  # 6
            [ResBlock(1280, 1280, 640), SpatialTransformer(640, 768, 8, 80)], [ResBlock(960, 1280, 640), SpatialTransformer(640, 768, 8, 80), Upsample(640)], [ResBlock(960, 1280, 320), SpatialTransformer(320, 768, 8, 40)], [ResBlock(640, 1280, 320), SpatialTransformer(320, 768, 8, 40)], [ResBlock(640, 1280, 320), SpatialTransformer(320, 768, 8, 40)], ]
        self.out = [tfa.layers.GroupNormalization(epsilon = 1e-5), tf.keras.activations.swish, get_conv2d(320, 4, kernel_size = 3, padding = 1)]
